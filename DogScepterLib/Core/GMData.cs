...
public class GMData
{
    ...
    public class GMVersionInfo
    {
        ...
        /// <summary>
        /// TODO: Whether the GameMaker data file uses Pre-Create events for rooms and objects.
        /// </summary>
        ...
        /// <summary>
        /// Whether the audio group is builtin to the GameMaker data file.
        /// </summary>
        //TODO: shouldn't this be a bool, considering it can only hold values 0 and 1 and that at a low level those two are the same?
        ...
        /// </summary>
        public void SetVersionNumber(int major = 1, int minor = 0, int release = 0, int build = 0)
        {
            if (Major < major)
            {
                Major = major;
    ...
    /// </summary>
    //TODO: make this <GMChunkNames, GMChunk> in order to mitigate typos?
    ...
    public GMString DefineString(string content, out int index)
    {
        ...
        lock (list)
        {
            if (StringCache != null)
            {
                ...
                for (int i = StringCache.Count; i < list.Count; i++)
                {
                    if (list[i].Content != content) continue;
